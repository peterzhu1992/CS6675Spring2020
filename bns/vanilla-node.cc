...
void VanillaNode::DoDispose(void)
{
    NS_LOG_FUNCTION(this);
...
// Application Methods
void VanillaNode::StartApplication() // Called at time specified by Start
{
    NS_LOG_FUNCTION(this);
...
void VanillaNode::StopApplication() // Called at time specified by Stop
{
    NS_LOG_FUNCTION(this);
...
void VanillaNode::InitListenSocket(void)
{
    NS_LOG_FUNCTION(this);
...
void VanillaNode::InitOutgoingConnection(void)
{
    NS_LOG_FUNCTION(this);
...
void VanillaNode::InitBroadcast(Block &b)
{
    NS_LOG_FUNCTION(this);
...
void VanillaNode::HandleAccept(ns3::Ptr<ns3::Socket> socketPtr, const ns3::Address &from)
{
    NS_LOG_FUNCTION(this);
...
void VanillaNode::HandleConnect(ns3::Ptr<ns3::Socket> socketPtr)
{
    NS_LOG_FUNCTION(this);
    if (!m_isRunning)
        return;
...
void VanillaNode::SendPacket(ns3::Ptr<ns3::Socket> socketPtr, ns3::Ptr<ns3::Packet> packet)
{
...
void VanillaNode::SendAvailable(ns3::Ptr<ns3::Socket> socketPtr)
{
    NS_LOG_FUNCTION(this);

    ns3::Ipv4Address peerAddr = GetSocketAddress(socketPtr);
    if (m_sendQueues.count(peerAddr) == 0)
        return;
    if (m_sendQueues[peerAddr].empty())
        return;

    uint32_t availBytes = socketPtr->GetTxAvailable();
    ns3::Ptr<ns3::Packet> next = m_sendQueues[peerAddr].front();

    if (availBytes < next->GetSize())
        return;
...
void VanillaNode::SendInvMessage(ns3::Ptr<ns3::Socket> socketPtr, std::vector<uint64_t> inventory)
{
    //ns3::Ipv4Address peerAddr = GetSocketAddress (socketPtr);
...
void VanillaNode::SendGetHeadersMessage(ns3::Ptr<ns3::Socket> socketPtr, uint64_t startID, uint64_t stopID)
{
    //ns3::Ipv4Address peerAddr = GetSocketAddress (socketPtr);
...
void VanillaNode::SendHeadersMessage(ns3::Ptr<ns3::Socket> socketPtr, std::vector<uint64_t> inventory)
{
    //ns3::Ipv4Address peerAddr = GetSocketAddress (socketPtr);
...
void VanillaNode::SendGetDataMessage(ns3::Ptr<ns3::Socket> socketPtr, std::vector<uint64_t> inventory)
{
    //ns3::Ipv4Address peerAddr = GetSocketAddress (socketPtr);
...
void VanillaNode::HandleSent(ns3::Ptr<ns3::Socket> socketPtr, uint32_t availBytes)
{
...
void VanillaNode::HandleRead(ns3::Ptr<ns3::Socket> socketPtr)
{
    ...
    if (m_recvQueues.count(peerAddr) == 0)
        m_recvQueues[peerAddr] = ns3::Create<ns3::Packet>();
...
void VanillaNode::HandlePeerClose(ns3::Ptr<ns3::Socket> socketPtr)
{
    NS_LOG_FUNCTION(this << socketPtr);
    if (!m_isRunning)
        return;
...
void VanillaNode::HandlePeerError(ns3::Ptr<ns3::Socket> socketPtr)
{
    NS_LOG_FUNCTION(this << socketPtr);
    if (!m_isRunning)
        return;
...
void VanillaNode::HandlePacket(ns3::Ptr<ns3::Socket> socketPtr, ns3::Ptr<ns3::Packet> packet)
{
    VanTypeHeader th;
    ...
    case VanMsgType::BLOCK:
        ...
        break;
    default:
        NS_LOG_DEBUG("Got unknown message: " << packet);
    }
}

void VanillaNode::ProcessPacket(ns3::Ptr<ns3::Socket> socketPtr)
{
    NS_LOG_FUNCTION(this);
    ...
    VanLengthHeader lh;
    if (curPacket->GetSize() < lh.GetSerializedSize())
        return; // we can't go on further, not enough to read length header
    ...
    if (curPacket->GetSize() < lh.GetSerializedSize() + length)
        return; // we can't go on further, not enough for the whole package
    ...
    if (curPacket->GetSize() > 0)
        ProcessPacket(socketPtr); // repeat for next packet
}

void VanillaNode::HandleInvMessage(ns3::Ptr<ns3::Socket> socketPtr, ns3::Ptr<ns3::Packet> packet)
{
    ns3::Ipv4Address peerAddr = GetSocketAddress(socketPtr);
    VanInvHeader ih;
...
void VanillaNode::HandleGetDataMessage(ns3::Ptr<ns3::Socket> socketPtr, ns3::Ptr<ns3::Packet> packet)
{
    ns3::Ipv4Address peerAddr = GetSocketAddress(socketPtr);
    VanGetDataHeader gdh;
...
void VanillaNode::HandleGetHeadersMessage(ns3::Ptr<ns3::Socket> socketPtr, ns3::Ptr<ns3::Packet> packet)
{
    VanGetHeadersHeader ghh;
    ...
    if (!m_blockchain->HasBlock(startID))
        startID = 0;

    uint64_t stopID = ghh.GetStopId();
    // find stopID. if not, assume topBlock.
    if (!m_blockchain->HasBlock(stopID))
        stopID = m_blockchain->GetTopBlockID();
    if (stopID == startID)
        return; // do not send anything, if we do not know anything newer
...
void VanillaNode::HandleHeadersMessage(ns3::Ptr<ns3::Socket> socketPtr, ns3::Ptr<ns3::Packet> packet)
{
    ns3::Ipv4Address peerAddr = GetSocketAddress(socketPtr);
    VanHeadersHeader hh;
...
void VanillaNode::HandleGetBlocksMessage(ns3::Ptr<ns3::Socket> socketPtr, ns3::Ptr<ns3::Packet> packet)
{
...
void VanillaNode::HandleBlockMessage(ns3::Ptr<ns3::Socket> socketPtr, ns3::Ptr<ns3::Packet> packet)
{
    //ns3::Ipv4Address peerAddr = GetSocketAddress(socketPtr);
    ...
    Block newBlock = Blockchain::GetNewBlock(newBlockID, newPrevID, newBlockSize);
        ...
        SetTTFB(newBlockID, ns3::Simulator::Now());
        SetTTLB(newBlockID, ns3::Simulator::Now());
...
void VanillaNode::SetBlockKnown(ns3::Ipv4Address peerAddr, uint64_t blockID)
{
...
bool VanillaNode::PeerKnowsBlock(ns3::Ipv4Address peerAddr, uint64_t blockID)
{
    std::vector<ns3::Ipv4Address> &alreadyKnown = m_knownBlocks[blockID];
    auto it = std::find(std::begin(alreadyKnown), std::end(alreadyKnown), peerAddr);
