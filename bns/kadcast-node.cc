...
// Application Methods
void 
KadcastNode::StartApplication ()    // Called at time specified by Start
{
    ...
    for (auto it : m_knownAddresses) {
        if (it != m_address) ns3::Simulator::Schedule (ns3::Seconds(p->GetValue()), &KadcastNode::SendPingMessage, this, it);
    }
...
void 
KadcastNode::StopApplication ()     // Called at time specified by Stop
{
    NS_LOG_FUNCTION (this);
...
void 
KadcastNode::UpdateBucket (ns3::Ipv4Address addr, nodeid_t nodeID)
{
    NS_LOG_FUNCTION (this);
...
void 
KadcastNode::InitBroadcast (Block& b)
{
...
void
KadcastNode::BroadcastBlock (Block& b)
{
    ...
    for (uint16_t bIndex = height-1; bIndex >= 0 && bIndex < KAD_ID_LEN; --bIndex) {
        ...
        if (m_buckets[bIndex].size() == 0) continue;
        ...
        while (toQuery > 0) {
            ...
            if (nodeAddr == m_address) continue;
...
void
KadcastNode::SendBlock (ns3::Ipv4Address &outgoingAddress, Block& b, uint16_t height)
{
...
void 
KadcastNode::HandleRead (ns3::Ptr<ns3::Socket> socket)
{
...
void 
KadcastNode::HandleSent (ns3::Ptr<ns3::Socket> socketPtr, uint32_t availBytes)
{
...
KadcastNode::SendAvailable()
{
    ...
    for (uint32_t i = 0; i < toSend; i++) {
        ...
        if(sent == -1) NS_LOG_WARN("Error sending packet: " << show_errno(m_socket->GetErrno()));
        if(sent != (int32_t) nextSize) NS_LOG_WARN("Couldn't send whole packet! Sent " << sent << " / " << nextSize << " bytes.");
...
void 
KadcastNode::HandlePingMessage(ns3::Ipv4Address &senderAddr, nodeid_t& senderID)
{
    NS_LOG_FUNCTION (this);
...
void 
KadcastNode::HandlePongMessage(ns3::Ipv4Address &senderAddr, nodeid_t& senderID)
{
    NS_LOG_FUNCTION (this);
    UpdateBucket(senderAddr, senderID);
    return;
...
void 
KadcastNode::HandleFindNodeMessage(ns3::Ipv4Address &senderAddr, nodeid_t& senderID, nodeid_t& targetID)
{
    NS_LOG_FUNCTION (this);
    UpdateBucket(senderAddr, senderID);
...
void
KadcastNode::HandleNodesMessage(ns3::Ipv4Address &senderAddr, nodeid_t& senderID, nodeid_t& targetID, std::vector<bentry_t> &nodes)
{
    NS_LOG_FUNCTION (this);
    if (m_nodeLookups.count(targetID) == 0) return; // we aren't actually looking for this id
    ...
    for (bentry_t e : nodes) {
        ...
        if (queryMap.count(dist) == 1) continue; // we actually already know this node
...
void
KadcastNode::HandleChunkMessage(ns3::Ipv4Address &senderAddr, nodeid_t& senderID, Chunk c, uint16_t height)
{
    NS_LOG_FUNCTION (this);
    if (c.blockID == 0) return;
    ...
    if (m_seenBroadcasts[c.blockID][c.chunkID]) return;
    m_seenBroadcasts[c.blockID][c.chunkID] = true;

    if (m_doneBlocks[c.blockID]) return;
...
void
KadcastNode::HandleRequestMessage(ns3::Ipv4Address &senderAddr, nodeid_t& senderID, uint64_t blockID)
{
...
void
KadcastNode::SendPingMessage(ns3::Ipv4Address &outgoingAddress)
{
    NS_LOG_FUNCTION (this);

    if (outgoingAddress == m_address) return; // do not send to self
...
void
KadcastNode::SendPongMessage(ns3::Ipv4Address &outgoingAddress)
{
    NS_LOG_FUNCTION (this);
...
void
KadcastNode::SendFindNodeMessage(ns3::Ipv4Address &outgoingAddress, nodeid_t& targetID)
{
    NS_LOG_FUNCTION (this);
...
void 
KadcastNode::SendNodesMessage(ns3::Ipv4Address &outgoingAddress, nodeid_t& targetID, std::vector<bentry_t> &nodes)
{
    NS_LOG_FUNCTION (this);
...
void
KadcastNode::SendChunkMessage(ns3::Ipv4Address &outgoingAddress, Chunk c, uint16_t height)
{
    NS_LOG_FUNCTION (this);
...
void
KadcastNode::SendRequestMessage(ns3::Ipv4Address &outgoingAddress, uint64_t blockID)
{
    NS_LOG_FUNCTION (this);
...
void
KadcastNode::InitLookupNode (nodeid_t &targetID)
{
    NS_LOG_FUNCTION(this);
    ...
    if (m_nodeLookups.count(targetID) == 1) return; 
...
void 
KadcastNode::LookupNode (nodeid_t &targetID, bool queryAll)
{
    NS_LOG_FUNCTION(this);
    if (targetID == m_nodeID) return;
    if (m_nodeLookups.count(targetID) == 0) return; // Make sure we do not acidentally create new entries
    ...
    if (nQueried < KadcastNode::kadK)  return;
...
void
KadcastNode::RequestMissingBlock(ns3::Ipv4Address& senderAddr, uint64_t blockID)
{ 
...
void
KadcastNode::RefreshBuckets()
{
    NS_LOG_FUNCTION (this);
    for (short i = 0; i < KAD_ID_LEN; ++i) {
        auto it = m_activeBuckets.find(i);
        if (it != std::end(m_activeBuckets)) continue;
        uint64_t min = pow2(i);
...
void
KadcastNode::RefreshNode(ns3::Ipv4Address &oldAddress, nodeid_t oldID, ns3::Ipv4Address &newAddress, nodeid_t newID)
{
    NS_LOG_FUNCTION (this);
...
void 
KadcastNode::RefreshTimeoutExpired (ns3::Ipv4Address &addr, nodeid_t nodeID)
{
    NS_LOG_FUNCTION(this);
    ...
    if (rit != std::end(m_pendingRefreshes)) {
        ...
        if (newID != m_nodeID) bucket.push_back(std::make_pair(newAddr, newID));
        m_pendingRefreshes.erase(rit);
    }
...
void
KadcastNode::MarkQueried (nodeid_t& targetID, nodeid_t& nodeID)
{
    if (m_nodeLookups.count(targetID) == 0) return; // Make sure we do not acidentally create new entries

    uint64_t dist = Distance(nodeID, targetID);
    if (m_nodeLookups[targetID].count(dist) == 0) return; // Make sure we do not acidentally add to the k closest nodes

    std::get<2>(m_nodeLookups[targetID][dist]) = true;
}

bool
KadcastNode::IsQueried (nodeid_t& targetID, nodeid_t& nodeID)
{
    if (m_nodeLookups.count(targetID) == 0) return false; // Make sure we do not acidentally create new entries

    uint64_t dist = Distance(nodeID, targetID);
    if (m_nodeLookups[targetID].count(dist) == 0) return false; // Make sure we do not acidentally add to the k closest nodes

    return std::get<2>(m_nodeLookups[targetID][dist]);
}

void
KadcastNode::PrintBuckets()
{
...
KadcastNode::Chunkify (Block b)
{
    ...
    if (b.blockSize % packetSize != 0) nChunks++; // one more chunk
...
Block
KadcastNode::Dechunkify (std::map<uint16_t, Chunk> chunks)
{
...
void 
KadcastNode::TerminateLookup(nodeid_t& targetID)
{
    if(m_nodeLookups.count(targetID) == 0) return; 
    m_nodeLookups.erase(targetID);
}
}
